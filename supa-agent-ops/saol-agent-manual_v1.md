# Supabase Agent Ops Library (SAOL) - AI Agent Manual v1.0
...
| Section | Line |
|---------|------|
| [Quick Start](#quick-start) | 30 |
| [Environment Setup](#environment-setup) | 55 |
| [Core Concepts](#core-concepts) | 75 |
| [Import Operations](#import-operations) | 110 |
| [Query Operations](#query-operations) | 210 |
| [Export Operations](#export-operations) | 285 |
| [Delete Operations](#delete-operations) | 355 |
| [Schema Operations](#schema-operations) | 415 |
| [RPC Operations](#rpc-operations) | 520 |
| [Maintenance Operations](#maintenance-operations) | 590 |
| [Verification Operations](#verification-operations) | 680 |
| [Error Handling](#error-handling) | 760 |
| [Usage Patterns by Agent Type](#usage-patterns-by-agent-type) | 840 |
| [Quick Reference Tables](#quick-reference-tables) | 920 |
| [Common Pitfalls & Solutions](#common-pitfalls--solutions) | 1010 |
...
- **Schema Guide:** `SCHEMA_OPERATIONS_GUIDE.md`
...
# APPENDIX: Brief Instruction Block for Copy-Paste

**Use this text block in all coding prompts to instruct agents on Supabase manipulation:**

---

## 🔧 Supabase Database Operations

**IMPORTANT:** For ALL Supabase database operations (CRUD, schema, maintenance), use the **Supabase Agent Ops Library (SAOL)**:
...
**Documentation:**
- 📘 **Agent Manual:** `saol-agent-manual_v1.md` (comprehensive guide, use line numbers in TOC)
- ⚡ **Quick Start:** `QUICK_START.md` (5-minute getting started)
- 📝 **Examples:** `EXAMPLES.md` (code patterns and use cases)
- 🚨 **Error Codes:** `ERROR_CODES.md` (error reference with solutions)
...
// Import/Upsert (Line 110 in manual)
await saol.agentImportTool({ source, table, mode: 'upsert', onConflict: 'id' });

// Query (Line 210 in manual)
await saol.agentQuery({ table, where: [...], limit: 100 });

// Export (Line 285 in manual)
await saol.agentExportData({ table, destination, config: { format: 'jsonl' } });

// Delete (Line 355 in manual)
await saol.agentDelete({ table, where: [...], confirm: true });

// Schema (Line 415 in manual)
await saol.agentIntrospectSchema({ table, transport: 'pg' });
await saol.agentExecuteDDL({ sql, dryRun: true, transport: 'pg' });

// Maintenance (Line 590 in manual)
await saol.agentVacuum({ table, analyze: true });
```

**Critical Rules:**
1. ✅ **Never manually escape strings** - library handles ALL special characters automatically
2. ✅ **Always use SERVICE_ROLE_KEY** - NOT anon key (see `saol-agent-manual_v1.md` Line 55)
3. ✅ **Run preflight checks** - `agentPreflight({ table })` before operations
4. ✅ **Use dry-run mode** - validate before executing destructive operations
5. ✅ **Check result.success** - always handle failures and follow `nextActions`
6. ✅ **Context efficiency** - manual designed for low token usage with direct line references
...
**Agent Workflow:**
1. Read relevant section from `saol-agent-manual_v1.md` (use TOC line numbers)
2. Run preflight check with `agentPreflight()`
3. Execute operation (use dry-run if destructive)
4. Check `result.success` and follow `nextActions` for errors
5. **Never** construct manual SQL with string concatenation

**For detailed documentation on any operation, refer to `saol-agent-manual_v1.md` with line numbers provided in the Table of Contents.**
