...
export async function getTemplatePerformanceMetrics(
  ...
  try {
    // Fetch all conversations for this template
    ...
    const successfulTests = conversations.filter(c => 
      c.status === 'approved' || c.status === 'generated'
    ).length;
    const failedTests = conversations.filter(c => 
