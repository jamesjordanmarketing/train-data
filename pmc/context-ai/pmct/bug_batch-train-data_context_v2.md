...
**Critical Rules:**
1. Never manually escape strings (SAOL handles automatically)
2. Always use SERVICE_ROLE_KEY (not anon key)
3. Run `agentPreflight({ table })` before operations
...
// Query data
await saol.agentQuery({ table: 'conversations', where: [...], limit: 10 });

// Check schema
await saol.agentIntrospectSchema({ table: 'conversations', transport: 'pg' });

// Count records
await saol.agentCount({ table: 'conversations', where: [...] });
```

**Full Manual:** `C:\Users\james\Master\BrightHub\brun\train-data\supa-agent-ops\saol-agent-manual_v2.md` (lines 30-1280, TOC at top)

**Transport Selection:** Use `supabase` for data ops, `pg` for schema ops, `rpc` for custom SQL
...
**Full Context:** See `C:\Users\james\Master\BrightHub\brun\train-data\pmc\context-ai\pmct\bug_batch-train-data_context_v1.md` for comprehensive details including user journeys, mock data validation, common bug patterns, and debugging approaches.
