...
async function main() {
  try {
    ...
    // First handle overview document
    console.log('\nGenerating Overview Prompt:');
    console.log('===========================');
    
    const overviewDoc = config.documents[0];
    displayHeader(projectName, projectAbbrev, overviewDoc);
    
    const overviewPaths = await getReferencePaths(overviewDoc, projectAbbrev, projectName);
    const overviewPrompt = await generatePrompt(overviewDoc, overviewPaths);
    
    if (overviewPrompt) {
      // Generate filename for the overview prompt
      const overviewFilename = `01-product-${projectAbbrev}-${overviewDoc.type}-prompt-v1.md`;
      
      console.log(`\nGenerated ${documentNames[overviewDoc.type]} Prompt:`);
      console.log('============================');
      savePromptToFile(overviewPrompt, overviewFilename, projectAbbrev);
      console.log('============================\n');
    } else {
      console.error('Failed to generate prompt for', overviewDoc.type);
      rl.close();
      return;
    }
    
    // Ask if user wants to proceed with user stories
    const proceed = await question('Would you like to generate the user stories prompt now? (y/n) > ');
    
    if (proceed.toLowerCase() === 'y' || proceed.trim() === '') {
      // Now handle user stories document
      console.log('\nGenerating User Stories Prompt:');
      console.log('===============================');
      ...
      if (userStoriesPrompt) {
        ...
        console.error('Failed to generate prompt for', userStoriesDoc.type);
      }
    }
